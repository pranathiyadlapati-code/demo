...
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;1,400&family=DM+Sans:wght@300;400;500;600&family=Cormorant+Garamond:ital,wght@0,300;0,600;1,300;1,600&family=Syne:wght@400;700;800&display=swap" rel="stylesheet"/>
...
.modal-box{background:#f2f2f2;border:1px solid rgba(212,160,23,.3);border-radius:20px;width:100%;max-width:820px;max-height:90vh;overflow-y:auto;box-shadow:0 30px 80px rgba(0,0,0,.7);}
.modal-header{display:flex;align-items:center;justify-content:space-between;padding:20px 28px;border-bottom:1px solid rgba(0,0,0,.12);background:#fff;border-radius:20px 20px 0 0;}
.modal-header h2{font-family:'Playfair Display',serif;font-size:1.15rem;color:#111;}
.modal-header h2 em{color:#C9A84C;font-style:italic;}
.modal-close{background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.15);color:#555;border-radius:8px;padding:6px 12px;cursor:pointer;font-size:.8rem;transition:all .2s;}
.modal-close:hover{background:rgba(255,80,60,.12);color:#cc3322;border-color:rgba(255,80,60,.3);}
.modal-body{padding:0;}

/* ─── PROFILE CARD — Design 1: Dark Luxury (profile2.php) ─── */
#profile-print-area{background:#f2f2f2;position:relative;}
...
.pc2-card > *{position:relative;z-index:1;}
...
.pc2-sb-photo img{width:100%;height:100%;object-fit:cover;}
.pc2-sb-name{
  font-family:'Cormorant Garamond',serif;
  font-size:1.5rem;
  color:#000;
...
.pc2-badge-wrap{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:4px;}
.pc2-badge{
  font-size:.52rem;
  padding:2px 9px;
  border-radius:2px;
...
.pc2-divider{height:1px;background:#000;margin:14px 0;opacity:.3;}
.pc2-sb-row{margin-bottom:12px;}
.pc2-sb-lbl{
  font-size:.48rem;
  letter-spacing:.18em;
  text-transform:uppercase;
...
.pc2-sb-val{font-size:.74rem;color:#000;line-height:1.5;font-family:'Syne',sans-serif;}
...
.pc2-mh-title span{color:#000;font-style:italic;}
.pc2-mh-year{
  font-size:.58rem;
  letter-spacing:.2em;
  color:#000;
...
/* Info section */
.pc2-info-section{margin-bottom:20px;}
.pc2-info-title{
  font-size:.5rem;
  letter-spacing:.22em;
  text-transform:uppercase;
...
.pc2-info-pairs{display:grid;grid-template-columns:1fr 1fr;gap:10px 18px;}
.pc2-ip{
  padding:11px 14px;
  background:#fff;
  border:2px solid #000;
...
.pc2-ip.pc2-full{grid-column:span 2;}
.pc2-ip-lbl{
  font-size:.48rem;
  letter-spacing:.15em;
  text-transform:uppercase;
...
.pc2-ip-val{font-size:.74rem;color:#000;line-height:1.5;font-family:'Syne',sans-serif;}
.pc2-pill{
...
/* Opinions */
.pc2-op-section{margin-top:4px;flex:1;}
.pc2-op-title{
  font-family:'Cormorant Garamond',serif;
  font-size:1.25rem;
  color:#000;
...
.pc2-op-title em{color:#000;font-style:italic;}
.pc2-op-count{
  font-size:.52rem;
  letter-spacing:.15em;
  font-family:'Syne',sans-serif;
...
.pc2-op-line{width:40px;height:2px;background:#000;margin-bottom:12px;}
.pc2-op-card{
  border:2px solid #000;
  border-radius:6px;
  padding:13px 16px;
...
.pc2-op-author{display:flex;align-items:center;gap:10px;margin-bottom:7px;}
.pc2-op-ava{
  width:28px;height:28px;border-radius:50%;background:#fff;border:2px solid #000;
  display:flex;align-items:center;justify-content:center;
  font-size:.6rem;font-weight:800;color:#000;flex-shrink:0;font-family:'Syne',sans-serif;
}
.pc2-op-name{font-size:.7rem;color:#000;font-weight:700;display:block;font-family:'Syne',sans-serif;}
.pc2-op-id{font-size:.56rem;color:#444;font-family:monospace;display:block;}
.pc2-op-text{
  font-family:'Cormorant Garamond',serif;
  font-size:.88rem;color:#000;line-height:1.7;font-style:italic;
}
.pc2-op-empty{
  text-align:center;padding:18px 12px;color:#666;font-size:.75rem;
...
.pc2-footer-text{font-size:.5rem;letter-spacing:.15em;text-transform:uppercase;color:#000;font-family:'Syne',sans-serif;}
.pc2-footer-roman{font-family:'Cormorant Garamond',serif;font-size:1.05rem;color:#000;}

.pc-modal-btns{padding:16px 28px;border-top:1px solid rgba(0,0,0,.1);background:#fff;display:flex;gap:10px;justify-content:flex-end;border-radius:0 0 20px 20px;}
</style>
</head>
<body>
...
<div id="pdf-render-holder" style="position:fixed;left:-9999px;top:0;width:794px;z-index:-999;pointer-events:none;background:#f2f2f2;"></div>
...
// ═══════════════════════════════════════════════════════════════════════════
// PROFILE CARD HTML — Design 1: Dark Luxury (profile2.php format)
// ═══════════════════════════════════════════════════════════════════════════
function buildProfileHTML(d) {
    ...
    var photo = '';
    if (d.photo_path) {
        photo = '<img src="' + _e(d.photo_path) + '" alt="Photo" crossorigin="anonymous" '
    ...
    if (d.exam) {
        ...
        for (var ei = 0; ei < exams.length; ei++) {
            ...
            if (et) examHTML += '<span class="pc2-pill">' + _e(et) + '</span>';
        }
    }

    // Opinions
    var opinions = d.opinions || [];
    var opCount  = opinions.length;
    var opinionsHTML = '';
    if (opCount) {
        for (var oi = 0; oi < opCount; oi++) {
            var op = opinions[oi];
            var nameParts = (op.author_name || op.author_id || '?').trim().split(' ').slice(0, 2);
            var initials  = '';
            for (var ni = 0; ni < nameParts.length; ni++) {
                initials += (nameParts[ni].charAt(0) || '').toUpperCase();
            }
            opinionsHTML +=
                '<div class="pc2-op-card">' +
                    '<div class="pc2-op-author">' +
                        '<div class="pc2-op-ava">' + _e(initials) + '</div>' +
                        '<div>' +
                            '<span class="pc2-op-name">' + _e(op.author_name || op.author_id) + '</span>' +
                            '<span class="pc2-op-id">'   + _e(op.author_id)  + '</span>' +
                        '</div>' +
                    '</div>' +
                    '<div class="pc2-op-text">' + _e(op.opinion) + '</div>' +
                '</div>';
        }
    } else {
        opinionsHTML = '<div class="pc2-op-empty">No opinions written yet.</div>';
    }

    // Batch display (e.g. "2021-2025" from year field)
    var batchDisplay = v('year') || '';

    // Info pairs — only render if value present
    var infoPairs = '';

    if (d.department) {
        infoPairs += '<div class="pc2-ip"><div class="pc2-ip-lbl">Department</div><div class="pc2-ip-val">' + v('department') + '</div></div>';
    }
    if (d.company) {
        infoPairs += '<div class="pc2-ip"><div class="pc2-ip-lbl">Company / Plans</div><div class="pc2-ip-val">' + v('company') + '</div></div>';
    }
    if (d.location && !d.department) {
        // fill gap if no department
        infoPairs += '<div class="pc2-ip"><div class="pc2-ip-lbl">Location</div><div class="pc2-ip-val">' + v('location') + '</div></div>';
    }
    if (d.location && d.department) {
        infoPairs += '<div class="pc2-ip"><div class="pc2-ip-lbl">Location</div><div class="pc2-ip-val">' + v('location') + '</div></div>';
    }
    if (examHTML) {
        infoPairs += '<div class="pc2-ip pc2-full"><div class="pc2-ip-lbl">Competitive Exams</div><div class="pc2-ip-val">' + examHTML + '</div></div>';
    }
    if (d.extra) {
        infoPairs += '<div class="pc2-ip pc2-full"><div class="pc2-ip-lbl">Achievements &amp; Extra-Curriculars</div><div class="pc2-ip-val">' + v('extra') + '</div></div>';
    }
    if (d.fav && !d.extra) {
        // show hobbies in main area only if no extra (otherwise hobbies go in sidebar)
        infoPairs += '<div class="pc2-ip pc2-full"><div class="pc2-ip-lbl">Hobbies &amp; Interests</div><div class="pc2-ip-val">' + v('fav') + '</div></div>';
    }
    ...
    return (
        '<div class="pc2-card">' +
            '<div class="pc2-inner">' +
                ...
                /* ── LEFT SIDEBAR ── */
                '<aside class="pc2-sidebar">' +


                // 18-05-2026
                    '<div class="pc2-sb-label">Slam Book</div>' +
                    '<div class="pc2-sb-uni">Vignan\'s University<br>Vadlamudi</div>' +


                        // '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">' +
                        //     '<div class="pc2-sb-label" style="margin-bottom:0;">Slam Book</div>' +
                        //     '<img src="vignan_logo1.svg" crossorigin="anonymous" alt="Vignan Logo" ' +
                    ...
                    '<div class="pc2-sb-photo">' + photo + '</div>' +

                    '<div class="pc2-sb-name">' + (v('name') || '&mdash;') + '</div>' +
                    (d.nickname
                        ? '<div class="pc2-sb-nick">&ldquo; ' + v('nickname') + ' &rdquo;</div>'
                        : '') +

                    '<div class="pc2-badge-wrap">' +
                        (d.bm      ? '<span class="pc2-badge">' + v('bm')      + '</span>' : '') +
                        (d.program ? '<span class="pc2-badge">' + v('program') + '</span>' : '') +
                        (d.branch  ? '<span class="pc2-badge">' + v('branch')  + '</span>' : '') +
                    '</div>' +

                    '<div class="pc2-divider"></div>' +

                    (d.regno
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Reg. Number</div><div class="pc2-sb-val">' + v('regno') + '</div></div>'
                        : '') +
                    (d.dob
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Date of Birth</div><div class="pc2-sb-val">' + v('dob') + '</div></div>'
                        : '') +
                    (d.mobile
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Mobile</div><div class="pc2-sb-val">' + v('mobile') + '</div></div>'
                        : '') +
                    (d.alt_mobile
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Alt. Mobile</div><div class="pc2-sb-val">' + v('alt_mobile') + '</div></div>'
                        : '') +
                    (d.location
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Location</div><div class="pc2-sb-val">' + v('location') + '</div></div>'
                        : '') +
                    (batchDisplay
                        ? '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Batch</div><div class="pc2-sb-val">' + batchDisplay + '</div></div>'
                        : '') +

                    (d.fav
                        ? '<div class="pc2-divider"></div>' +
                          '<div class="pc2-sb-row"><div class="pc2-sb-lbl">Hobbies &amp; Interests</div><div class="pc2-sb-val">' + v('fav') + '</div></div>'
                        : '') +
                '</aside>' +
                ...
                // 18-05-2026
                    // '<div class="pc2-main-header">' +
                    //     '<div class="pc2-mh-title">Profile<br><span>Card</span></div>' +
                    //     '<div class="pc2-mh-year">Batch Year<br>' + (batchDisplay || '&mdash;') + '</div>' +
                    // '</div>' +
...
'</div>' +
                    ...
                    (infoPairs
                        ? '<div class="pc2-info-section">' +
                              '<div class="pc2-info-title">Academic &amp; Career</div>' +
                              '<div class="pc2-info-pairs">' + infoPairs + '</div>' +
                          '</div>'
                        : '') +

                    '<div class="pc2-op-section">' +
                        '<div class="pc2-op-title">Friends\' <em>Opinions</em> <span class="pc2-op-count">' + opCount + ' note' + (opCount !== 1 ? 's' : '') + '</span></div>' +
                        '<div class="pc2-op-line"></div>' +
                        opinionsHTML +
                    '</div>' +

                    '<div class="pc2-footer">' +
                        '<span class="pc2-footer-text">Vignan\'s University &middot; Official Slam Book</span>' +
                    '</div>' +
                '</main>' +
...
function renderStudentToPDF(studentData) {
    ...
    return new Promise(function(resolve, reject) {
        setTimeout(function() {
            var cardEl = holder.querySelector('.pc2-card');
