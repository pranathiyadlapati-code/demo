...
.bs-rs-batch{
  margin-top:18px;
  padding:18px 16px;
  background:linear-gradient(135deg,#111 0%,#1b1b1b 55%,#D4820A 140%);
  border:2px solid #D4820A;
  /* border-radius:14px; */
  position:relative;
  overflow:hidden;
  box-shadow:0 10px 24px rgba(212,130,10,.18);
}
    ...
    <div class="content">
      ...
      <div id="charts">
        ...
        <div class="charts-row">
          ...
          <div class="chart-card">
            <h3>Exit Feedback Analytics Report - Overall</h3>
          ...
          <div class="chart-card">
            <h3>Exit Feedback Analytics &mdash; Section-wise</h3>
...
<!-- 20-05-2026 -->
            ...
            <span style="font-size:.7rem;color:var(--muted);font-weight:400;text-transform:none;letter-spacing:0;">(If all 3 sections are completed)</span>
...
<!-- 20-05-2026 -->
            ...
            <span style="font-size:.7rem;color:var(--muted);font-weight:400;text-transform:none;letter-spacing:0;">(at least one section is not complete)</span>
...
function loadDetails(f) {
    ...
    fullySelectedSet = null;

    fetch(AJAX_FILE + '?action=fully&' + qs)
        .then(function(r){ return r.json(); })
        .then(function(d){
            fullyData = Array.isArray(d) ? d : [];
            fullySelectedSet = new Set(fullyData.map(function(r){ return r.regno; }));
    ...
    partialSelectedSet = null;

    fetch(AJAX_FILE + '?action=partial&' + qs)
        .then(function(r){ return r.json(); })
        .then(function(d){
            partialData = Array.isArray(d) ? d : [];
            partialSelectedSet = new Set(partialData.map(function(r){ return r.regno; }));
...
/* ============================================================
   FIXED: toggleAllFully
   — writes to fullySelectedSet across ALL matching rows
     (not just the current page's DOM checkboxes)
   — calls renderFullyTable() so the DOM reflects the Set
   ============================================================ */
function toggleAllFully(masterCb) {
    var src = fullySearch
        ? fullyData.filter(function(r){
            return (r.regno||'').toLowerCase().indexOf(fullySearch) > -1 ||
                   (r.name||'').toLowerCase().indexOf(fullySearch) > -1;
          })
        : fullyData;

    if (masterCb.checked) {
        src.forEach(function(r){ fullySelectedSet.add(r.regno); });
    } else {
        src.forEach(function(r){ fullySelectedSet.delete(r.regno); });
    }
    renderFullyTable();
}
...
function renderFullyTable(){
    ...
    /* Count selected across ALL matching rows (not just this page) */
    var selectedCount = fullySelectedSet ? src.filter(function(r){ return fullySelectedSet.has(r.regno); }).length : 0;
    var allSelected   = selectedCount === total && total > 0;
    var someSelected  = selectedCount > 0 && selectedCount < total;

    var bulkBar = '<div class="bulk-bar">' +
      '<div class="bulk-bar-left">' +
        '<label class="bulk-sel-label">' +
          '<input type="checkbox" id="fully-check-all" ' + (allSelected ? 'checked' : '') + ' onchange="toggleAllFully(this)">' +
          ' Select / Deselect All' +
        '</label>' +
        '<span class="bulk-count" id="fully-selected-count"><strong>' + selectedCount + '</strong> of ' + total + ' selected</span>' +
      '</div>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
        ...
        '<button class="btn-bulk-zip" id="fully-bulk-btn" ' + (selectedCount === 0 ? 'disabled' : '') + ' onclick="bulkDownloadZip()">' +
          '<span class="zip-icon">&#128230;</span>' +
          (selectedCount > 0 ? 'Download ' + selectedCount + ' PDF' + (selectedCount>1?'s':'') + ' as ZIP' : 'Bulk Download ZIP') +
        '</button>' +
      '</div>' +
    '</div>';
    ...
    '<div class="table-scroll"><table class="data-table"><thead><tr>' +
      '<th style="width:36px;text-align:center;"><input type="checkbox" id="fully-check-all-hdr" ' + (allSelected ? 'checked' : '') + ' style="accent-color:var(--gold);width:15px;height:15px;cursor:pointer;" onchange="toggleAllFully(this);"></th>' +
    ...
    /* Sync indeterminate state after render */
    var barCb = document.getElementById('fully-check-all');
    var hdrCb = document.getElementById('fully-check-all-hdr');
    if (barCb) barCb.indeterminate = someSelected;
    if (hdrCb) hdrCb.indeterminate = someSelected;
}
...
/* ============================================================
   FIXED: toggleAllPartial
   — writes to partialSelectedSet across ALL matching rows
     (not just the current page's DOM checkboxes)
   — calls renderPartialTable() so the DOM reflects the Set
   ============================================================ */
function toggleAllPartial(masterCb) {
    ...
    if (masterCb.checked) {
        src.forEach(function(r){ partialSelectedSet.add(r.regno); });
    } else {
        src.forEach(function(r){ partialSelectedSet.delete(r.regno); });
    }
    renderPartialTable();
}
...
function updatePartialSelectedCount() {
    ...
    var checked = partialSelectedSet
        ? src.filter(function(r){ return partialSelectedSet.has(r.regno); }).length
        : 0;
    ...
    if (el) {
        el.innerHTML = '<strong>' + checked + '</strong> of ' + total + ' selected';
    }
...
function renderPartialTable(){
    ...
    /* Count selected across ALL matching rows (not just this page) */
    var selectedCount = partialSelectedSet ? src.filter(function(r){ return partialSelectedSet.has(r.regno); }).length : 0;
    var allSelected   = selectedCount === total && total > 0;
    var someSelected  = selectedCount > 0 && selectedCount < total;
    ...
    var bulkBar = '<div class="bulk-bar">' +
      '<div class="bulk-bar-left">' +
        '<label class="bulk-sel-label">' +
          '<input type="checkbox" id="partial-check-all" ' + (allSelected ? 'checked' : '') + ' onchange="toggleAllPartial(this)">' +
          ' Select / Deselect All' +
        '</label>' +
        '<span class="bulk-count" id="partial-selected-count"><strong>' + selectedCount + '</strong> of ' + total + ' selected</span>' +
      '</div>' +
      '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
        ...
        '<button class="btn-bulk-zip" id="partial-bulk-btn" ' + (selectedCount === 0 ? 'disabled' : '') + ' onclick="bulkDownloadZipPartial()">' +
          '<span class="zip-icon">&#128230;</span>' +
          (selectedCount > 0 ? 'Download ' + selectedCount + ' PDF' + (selectedCount>1?'s':'') + ' as ZIP' : 'Bulk Download ZIP') +
        '</button>' +
      '</div>' +
    ...
    bulkBar +
    '<div class="table-scroll"><table class="data-table"><thead><tr>' +
      '<th style="width:36px;text-align:center;"><input type="checkbox" id="partial-check-all-hdr" ' + (allSelected ? 'checked' : '') + ' style="accent-color:var(--gold);width:15px;height:15px;cursor:pointer;" onchange="toggleAllPartial(this);"></th>' +
...
function buildProfileHTML(d) {
    ...
    if (d.company)    info += '<tr><td class="bs-it-lbl" style="padding-right:30px;">Company</td><td class="bs-it-val">' + v('company') + '</td></tr>';
    if (d.location)   info += '<tr><td class="bs-it-lbl" style="padding-right:30px;">Location</td><td class="bs-it-val">' + v('location') + '</td></tr>';
    if (d.exam) {
        ...
        // if (eps) info += '<tr><td class="bs-it-lbl" style="padding-right:30px;">Exams</td><td class="bs-it-val">' + eps + '</td></tr>';
...
// 20-05-2026
    ...
    // if (d.fav)   info += '<tr><td class="bs-it-lbl" style="padding-right:30px;">Favourite Food</td><td class="bs-it-val">' + v('fav') + '</td></tr>';
    // if (d.extra) info += '<tr><td class="bs-it-lbl" style="padding-right:30px;">Extra Activities</td><td class="bs-it-val">' + v('extra') + '</td></tr>';


    if (d.fav)   info += '<tr><td class="bs-it-lbl">Favourite Food</td><td class="bs-it-val">' + v('fav') + '</td></tr>';
    if (d.extra) info += '<tr><td class="bs-it-lbl">Extra Activity</td><td class="bs-it-val">' + v('extra') + '</td></tr>';
    ...
    // 19-05-2026 height
    var sidebar = '<div class="bs-rs-photo" style="height:240px;">' + photoInner + '</div>';


    // 19-05-2026

    // if (d.branch || d.department) sidebar += '<div class="bs-rs-batch"><div class="bs-rs-batch-lbl">Branch</div><div class="bs-rs-batch-val" style="font-size:1.3rem;letter-spacing:.06em;font-weight:900;text-align:center;display:block;line-height:1.2;margin-top:4px;">' + (v('branch') || v('department')) + '</div></div>';

        if (d.year) sidebar += '<div class="bs-rs-batch"><div class="bs-rs-batch-lbl">Branch</div><div class="bs-rs-batch-val">' + esc(d.branch || d.specialization || '') + '</div></div>';
    ...
    return (
        '<div class="bs-card">' +
          '<div class="bs-inner">' +
// '<img src="logo1.svg.jpeg" crossorigin="anonymous" alt="Vignan Logo" style="position:absolute;top:160px;left:50px;width:104px;height:60px;object-fit:contain;" onerror="this.style.display=\'none\'">' +



// 20-05-2026

'<img src="logo1.jpeg" crossorigin="anonymous" alt="Vignan Logo" style="position:absolute;top:160px;left:50px;width:104px;height:60px;object-fit:contain;" onerror="this.style.display=\'none\'">' +
            ...
            '<div class="bs-masthead">' +
              ...
              '<div class="bs-mast-sub" style="justify-content:center;margin-top:8px;">' +
                '<strong style="font-size:1rem;letter-spacing:.14em;color:#D4820A;">Batch of ' + (year || '&mdash;') + '</strong>' +
              '</div>' +
            '</div>' +
...
//  '<div style="position:absolute;inset:0;pointer-events:none;overflow:hidden;z-index:5;">' +
//               (function() {
//                 var text = (year || 'Class') + ' \u00b7 ' + (d.regno || '') + ' \u00b7 ' + (d.branch || d.department || '');
//                 var tiles = '';
//                 for (var i = 0; i < 40; i++) {
//                   var top  = (Math.floor(i / 4) * 12) + (Math.random() * 4) + '%';
//                   var left = ((i % 4) * 25) + (Math.random() * 10) + '%';
//                   tiles +=
//                     '<div style="' +
//                       'position:absolute;' +
//                       'top:'  + top  + ';' +
...
'<div style="position:absolute;inset:0;pointer-events:none;overflow:hidden;z-index:5;display:flex;align-items:center;justify-content:center;">' +


// large logo faded in the background (comment out if it causes issues with PDF generation)
   '<img src="vignan_logo1.svg" crossorigin="anonymous" alt="" style="width:420px;height:420px;object-fit:contain;opacity:0.04;user-select:none;" onerror="this.style.display=\'none\'">' +


// small logo 

// '<img src="vignan_logo1.svg" crossorigin="anonymous" alt="" style="width:220px;height:220px;object-fit:contain;opacity:0.04;user-select:none;" onerror="this.style.display=\'none\'">' +
// '</div>' +
...
/* Bulk ZIP — Partially Filled */
function bulkDownloadZipPartial() {
    var src = partialSearch
        ? partialData.filter(function(r){ return (r.regno||'').toLowerCase().indexOf(partialSearch)>-1||(r.name||'').toLowerCase().indexOf(partialSearch)>-1; })
        : partialData;
        ...
        if (bulkCancelled || idx >= students.length) {
            ...
            zip.generateAsync({ type:'blob', compression:'DEFLATE', compressionOptions:{level:6} })
                .then(function(blob) {
                    ...
                    a.href  = url;
                    var ts  = new Date().toISOString().slice(0,10);
                    a.download = 'SlamBook_Partial_Bulk_' + ts + '.zip';
                    document.body.appendChild(a); a.click(); document.body.removeChild(a);
                    URL.revokeObjectURL(url);
                    subText.textContent = skipped > 0
                        ? 'Done! ' + (done-skipped) + ' PDFs zipped \u00b7 ' + skipped + ' skipped.'
