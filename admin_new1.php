...
/* stamp */
.bs-stamp{
  position:absolute; top:158px; right:56px;
...
.bs-mast-sub{
  ...
  font-size:.54rem; letter-spacing:.1em; color:#6A5F50; text-transform:uppercase;
}
.bs-mast-sub strong{ color:#D4820A; }
/* main grid */
...
.bs-tag-row{ display:flex; flex-wrap:wrap; gap:4px; }
.bs-tag{ font-size:.5rem; letter-spacing:.1em; text-transform:uppercase; font-weight:700; padding:2px 8px; border:1.5px solid #111; color:#111; }
...
.bs-info-table td{ padding:5px 0; vertical-align:top; }
.bs-it-lbl{ font-size:.48rem; letter-spacing:.15em; text-transform:uppercase; color:#6A5F50; font-weight:700; width:100px; padding-right:10px; padding-top:7px; }
.bs-it-val{ font-size:.74rem; color:#2A2520; line-height:1.5; }
...
/* right sidebar */
.bs-rs-photo{ width:100%; height:155px; background:rgba(200,190,172,.3); border:1.5px solid #111; display:flex; align-items:center; justify-content:center; font-size:3.2rem; margin-bottom:11px; overflow:hidden; }
...
function buildProfileHTML(d) {
    ...
    if (d.department) info += '<tr><td class="bs-it-lbl">Department</td><td class="bs-it-val">' + v('department') + '</td></tr>';
    if (d.year)       info += '<tr><td class="bs-it-lbl">Batch Year</td><td class="bs-it-val">' + year + '</td></tr>';
    if (d.company)    info += '<tr><td class="bs-it-lbl">Company / Plans</td><td class="bs-it-val">' + v('company') + '</td></tr>';
    ...
    if (opCount) {
        ...
        opsHTML = '<div class="bs-op-empty">No opinions written yet.</div>';
    }

    // ── Right sidebar
    var sidebar = '<div class="bs-rs-photo">' + photoInner + '</div>';
    if (d.mobile)     sidebar += '<div class="bs-rs-section"><div class="bs-rs-lbl">Mobile</div><div class="bs-rs-val">' + v('mobile') + '</div></div>';
    if (d.alt_mobile) sidebar += '<div class="bs-rs-section"><div class="bs-rs-lbl">Alt. Mobile</div><div class="bs-rs-val">' + v('alt_mobile') + '</div></div>';
    if (d.location)   sidebar += '<div class="bs-rs-section"><div class="bs-rs-lbl">Location</div><div class="bs-rs-val">' + v('location') + '</div></div>';
    ...
    return (
        '<div class="bs-card">' +
          '<div class="bs-inner">' +
            ...
            //   '<div class="bs-mast-sub">' +
            //     '<span>Official Student Profile Card</span>' +
            //     '<strong>Class of ' + (year || '&mdash;') + '</strong>' +
            //     '<span>' + _e(dept) + '</span>' +
            //   '</div>' +
            // '</div>' +
            // Masthead
'<div class="bs-masthead">' +
  ...
  '<div class="bs-mast-title-row" style="display:flex;align-items:center;justify-content:center;gap:14px;">' +
    '<img src="vignan_logo1.svg" crossorigin="anonymous" alt="Vignan Logo" style="width:48px;height:48px;object-fit:contain;" onerror="this.style.display=\'none\'">' +
    '<div class="bs-mast-title">SLAM BOOK</div>' +
    '</div>' +
  '<div class="bs-mast-sub">' +
    '<span>Official Student Profile Card</span>' +
